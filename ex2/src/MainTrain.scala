...
object MainTrain {
  ...
  def testTimeSeries():Unit={
    val ts=new TimeSeries("ex2/src/train.csv")
  ...
  def testZAD():Unit={
    val ts=new TimeSeries("ex2/src/train2.csv")
    ...
    val r1=ZAnomalyDetector.detect(model,new TimeSeries("ex2/src/test2.csv"))
    if(r1.length!=1)
  ...
  def testLRAD():Unit={
    val ts=new TimeSeries("ex2/src/train3.csv")
    val model=LinearRegAnomalyDetector.learn(ts)
    val r0=LinearRegAnomalyDetector.detect(model,ts)
    if(r0.length>0)
      println("there should not be any anomalies detected here (-10)")
    val r1=LinearRegAnomalyDetector.detect(model,new TimeSeries("ex2/src/test3.csv"))
    if(r1.length!=2)
  ...
  def testSSD():Unit={
    ...
    if(r0.length>0)
      println("there should not be any anomalies detected here (-10)")
    val r1=SumSqrAnomalyDetector.detect(model,new TimeSeries("ex2/src/test3.csv"))
    if(r1.length!=2)
      println("wrong number of reported anomalies (-10)")
  ...
  def testHAD():Unit={
    val ts=new TimeSeries("ex2/src/train4.csv")
    val model=HybridAnomalyDetector.learn(ts)
    val r0=HybridAnomalyDetector.detect(model,ts)

    if(r0.length>0)
      println("there should not be any anomalies detected here (-10)")
    val r1=HybridAnomalyDetector.detect(model,new TimeSeries("ex2/src/test4.csv"))
